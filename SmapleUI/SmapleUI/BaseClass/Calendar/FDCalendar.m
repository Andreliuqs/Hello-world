...
//#define Weekdays @[[NSString localName:@"SUN"], [NSString localName:@"MON"], [NSString localName:@"TUE"], [NSString localName:@"WED"], [NSString localName:@"THU"], [NSString localName:@"FRI"], [NSString localName:@"SAT"]]
...
- (UIView *)datePickerView {
    if (!_datePickerView) {
        ...
        [okButton setTitle:NSLocalizedString(@"Confirm", @"") forState:UIControlStateNormal];
...
- (void)setupTitleBar {
    ...
    titleView.backgroundColor = [UIColor lightGrayColor];//[UIColor colorWithPatternImage:[UIImage imageNamed:@"wl_tabbar_bg.png"]];//[UIColor lightGrayColor];
    [self addSubview:titleView];
    
    UIButton *leftButton = [[UIButton alloc] initWithFrame:CGRectMake(75, 10, 32, 24)];
    ...
    UIButton *rightButton = [[UIButton alloc] initWithFrame:CGRectMake(titleView.frame.size.width - 105, 10, 32, 24)];
    ...
    UIButton *searchButton = [[UIButton alloc] initWithFrame:CGRectMake(titleView.frame.size.width - 65, 10, 60, 24)];
    [searchButton setTitle:@"查看" forState:0];//查看
    searchButton.titleLabel.font = [UIFont systemFontOfSize:15.0];
    [searchButton addTarget:self action:@selector(actSearch) forControlEvents:UIControlEventTouchUpInside];
...
- (void)actSearch
{
    if(self.delegate && [self.delegate respondsToSelector:@selector(didSelecedSearchDate:)])
    {
        [self.delegate didSelecedSearchDate:self.date];
    }
}
...
- (void)calendarItem:(FDCalendarItem *)item didSelectedDate:(NSDate *)date {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(didSelecedCalendarDate:)]) {
        [self.delegate didSelecedCalendarDate:date];
    }
}
